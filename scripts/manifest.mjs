...
const names = ['adaptive-context.mjs', 'agent-context.mjs', 'discovery-schemas.mjs', 'reasoning-guidance.mjs', 'analysis-pass.mjs', 'final-review.mjs', 'target-model.mjs', 'audit.mjs'];
const files = names.map(name => ({ name, sha256: hash(readFileSync(resolve(root, 'runtime', name))) }));
const manifest = { packageVersion: '0.1.0', runtimeVersion: hash(JSON.stringify(files)).slice(0, 16), files };
